...
def write_loss_new(iterations, trainer, train_writer, output_dir):
    ...
    update_csv(total_loss, iterations, os.path.join(output_dir, 'total_loss.csv'))
    update_csv(gen_loss, iterations, os.path.join(output_dir, 'gen_loss.csv'))
    update_csv(dis_loss, iterations, os.path.join(output_dir, 'dis_loss.csv'))
    update_csv(sia_loss, iterations, os.path.join(output_dir, 'sia_loss.csv'))


def update_csv(loss, global_step, file_name):
    loss_new = {k: [float(loss[k].data.cpu())] for k in loss.keys()}
    loss_new['step'] = global_step
    df_append = DataFrame.from_dict(loss_new)
    df_append.set_index('step', inplace=True)

    if not os.path.isfile(file_name):
        df_append.to_csv(file_name)
    else:
        df_ori = pd.read_csv(file_name, index_col=0)
        df_ori = df_ori.append(df_append)
        df_ori.to_csv(file_name)
